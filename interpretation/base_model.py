...
class CIFlow_base(nn.Module):
    def __init__(self, args, x_dim, num_class, model_config):
        super().__init__()
        ...
        for _ in range(self.blocks_num):
            self.node_encoder_list.append(MLP(2, x_dim, 128,hidden_size))
        self.concat_mlp = MLP(2, self.power_num+1 ,self.power_num+1, 1)
        self.batch_norms = torch.nn.ModuleList()
        for _ in range(self.power_num+1):
    ...
    def forward(self,W,data, x,input_adj,H_all,targets, batch,is_test):
        ...
        if is_test==True:
            with torch.no_grad():
                ...
                all_nodes_set = set(range(x.shape[0]))
                unimportant_nodes_set = all_nodes_set - important_nodes_set
                ### plus 
                important_nodes_tensor = torch.tensor(list(important_nodes_set)).long()
                x_occluded = torch.clone(x)
                x_occluded[important_nodes_tensor] = 0
                pred_fidelity_1_m = self.fidelity_func(W, data, x_occluded)
                ### minus
                unimportant_nodes_tensor = torch.tensor(list(unimportant_nodes_set)).long()
                x_occluded = torch.clone(data.x)
                x_occluded[unimportant_nodes_tensor] = 0
                pred_fidelity_m = self.fidelity_func(W, data, x_occluded)
        else:
            pred_fidelity_m,pred_fidelity_1_m,sparsity,important_nodes =None,None,None,None
         

    


        return pred1,pred2,S_all,ind_positive_sample,pred_fidelity_m,pred_fidelity_1_m,sparsity,E_all,Q,important_nodes
    ...
    def get_emb(self, W,features):
        W_list = []
        W_list.append(W)
        for p in range(1, self.power_num):
        ...
        H_all = torch.reshape(H_all,(self.power_num+1,-1))
        H_all = H_all.t()
    ...
    def fidelity_func(self,W, data, new_x):
        W_list = []
        W_list.append(W)
        ...
        hidden_rep_tmp = []
        for i in range(self.blocks_num):
            node_feature_tmp = self.node_encoder_list[i](new_x)
            node_feature_list.append(node_feature_tmp)
        ...
        for i in range(len(unique)):
            ...
            count += len(ind)
        E_all = torch.stack(E_lists)
        E_sorted_list1 = E_all.reshape(E_all.shape[0],-1)
        pred1_1 = self.final(E_sorted_list1)      
        pred = F.dropout(pred1_1,self.dropout_p,training = False)
