...
class CIFlow(nn.Module):
    ...
    def __loss__(self, pred1, pred2, ind_positive_sample, Q, E, S, H, L, batch, labels):
        ...
        proto_loss = self.args.lambda_proto*self.ClusterPrototype_loss(Q,E,S,batch)
    ...
    def forward(self,is_test,A, L, W, data):
        ...
        # Compute prediction y1, prediction y2, correctly predicted sample in predition y1, important nodes, soft cluster assignment matrix, graph embeddings, prototype assignment matrix
        pred1, pred2, ind_positive_sample, important_nodes, S, E, Q  = self.base(W,data, data.x,A,H,data.y, data.batch,is_test)
        # Compute the losses
        loss,loss_dict = self.__loss__(pred1, pred2, ind_positive_sample, Q, E, S, H, L, data.batch, data.y)

        return loss, loss_dict, pred1, pred2, important_nodes
