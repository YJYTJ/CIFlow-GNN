...
class Mutag(InMemoryDataset):
    ...
    def process(self):
        with open(self.raw_dir + '/Mutagenicity.pkl', 'rb') as fin:
            _, original_features, original_labels = pkl.load(fin) #(4337,418,14) #(4337,2)
        ...
        for i in range(original_labels.shape[0]):
            ...
            data_list.append(Data(x=x, y=y, edge_index=edge_index, node_label=node_label, edge_label=edge_label, node_type=torch.tensor(node_type_lists[i]),ground_truth =node_label))

        data, slices = self.collate(data_list) #2951
