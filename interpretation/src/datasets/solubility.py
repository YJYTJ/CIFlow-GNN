...
def smiles_to_graph(smiles: str):
    ...
    nodes = molecule.nodes(data='element')
...
class SolubilityDataset(InMemoryDataset):
    ...
    def process(self):
        self.df = pd.read_csv(str(self.root)+'/delaney-processed.csv')
        ...
        for item in range(dataset_len):
            ...
            for atom in molecule.GetAtoms():
                if atom.GetSymbol() == "O":  # 选取氧原子
                    for neighbor in atom.GetNeighbors():
                        if neighbor.GetSymbol() == "H":
                            oh_positions.extend([atom.GetIdx(), neighbor.GetIdx()])
            
            if len(oh_positions) > 0 :
                if new_target ==1:
                    print('正确')
                    ...
                    # # 绘制分子
                    # rdMolDraw2D.PrepareAndDrawMolecule(drawer, molecule, highlightAtoms=oh_positions)
            ...
            solubility_nodes.append(item)
        np.save('solubility_nodes.npy',solubility_nodes)
