...
class Motif(InMemoryDataset):
    ...
    def __init__(self, root, mode, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.mode = mode
        super(Motif, self).__init__(root, transform, pre_transform, pre_filter)

        idx = self.processed_file_names.index('Motif_{}.pt'.format(mode))
    ...
    def processed_file_names(self):
        return ['Motif_train.pt', 'Motif_val.pt', 'Motif_test.pt']
    ...
    def process(self):
        ...
        for idx, (edge_index, y, ground_truth, z, p) in enumerate(zip(edge_index_list, label_list, ground_truth_list, role_id_list, pos)):
            ...
            # node_label = torch.tensor(z, dtype=torch.long)
            for kk in z:
                if kk not in label_dict:
                    mapped = len(label_dict)
                    label_dict[kk] = mapped

        node_feature_dim = len(label_dict)
        # enc = OneHotEncoder(sparse=False,categories=node_feature_dim)
        for idx, (edge_index, y, ground_truth, z, p) in enumerate(zip(edge_index_list, label_list, ground_truth_list, role_id_list, pos)):
            edge_index = torch.from_numpy(edge_index).long()
            ...
            # x[index, z] = 1
            x = torch.zeros((node_idx.size(0), node_feature_dim))
            x.scatter_(1, torch.unsqueeze(torch.tensor(z), 1), 1)
            ...
            data = Data(x=x, y=y, edge_index=edge_index, edge_attr=edge_attr, node_label=node_label, edge_label=edge_label,ground_truth =node_label )
        ...
        idx = self.processed_file_names.index('Motif_{}.pt'.format(self.mode))
