...
def args_parser():
    ...
    parser.add_argument('--latent_dim2', type=int, default=128, help='node_feature hidden')
    ...
    parser.add_argument('--K_cluster', type=int, default=4, help='K_cluster')
    parser.add_argument('--num_graph_filter', type=int, default=8, help='num_graph_filter')
    parser.add_argument('--result_folder',  type=str, default='results', help='results folder')
    parser.add_argument('--layer_num',  type=int, default=6, help='W layer_num')
...
def main(args): 
    ...
    validation_loss_all = np.zeros((args.epochs, args.max_fold-1))
    test_accuracy_all = np.zeros((args.epochs, args.max_fold-1))
    test_acc_all = np.zeros(args.max_fold-1)
    for num_fold in range(1,args.max_fold):
        ...
        Lsym_train,W_train = normalized_laplacian_W_batch(adj_train,args.layer_num)
        Lsym_val,W_val = normalized_laplacian_W_batch(adj_val,args.layer_num)
        Lsym_test,W_test = normalized_laplacian_W_batch(adj_test,args.layer_num)
        ...
        for epoch in range(args.epochs):
            ...
            validation_loss_all[epoch][num_fold-1] = val_loss
            test_accuracy_all[epoch][num_fold-1] = test_acc
    ...
    min_ind = np.argmin(validation_loss_all, axis=0)
    for i in range(args.max_fold-1):
        ind = min_ind[i]
        test_acc_all[i] = test_accuracy_all[ind][i]
    ave_acc = np.mean(test_acc_all)
...
if __name__ == "__main__":
    ...
    #"BZR","BZR_MD", "DHFR","COX2","NCI1","DD","IMDB-BINARY","IMDB-MULTI" 
    args = args_parser()
    
    # if dataset in ['MUTAG']:
    #     args.use_node_labels = True
    # else:
    #     args.use_node_labels = False
