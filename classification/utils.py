...
def load_data(ds_name, use_node_labels=False,use_node_attri=False):   
    ...
    directory = './datasets/'
    graph_indicator = np.loadtxt(directory+"%s/raw/%s_graph_indicator.txt"%(ds_name,ds_name), dtype=np.int64)
    _,graph_size = np.unique(graph_indicator, return_counts=True)
    
    edges = np.loadtxt(directory+"%s/raw/%s_A.txt"%(ds_name,ds_name), dtype=np.int64, delimiter=",")
    edges -= 1
    A = csr_matrix((np.ones(edges.shape[0]), (edges[:,0], edges[:,1])), shape=(graph_indicator.size, graph_indicator.size))
    
    if use_node_labels:
        x = np.loadtxt(directory+"%s/raw/%s_node_labels.txt"%(ds_name,ds_name), dtype=np.int64).reshape(-1,1)
        enc = OneHotEncoder(sparse=False)
        x = enc.fit_transform(x)
        if use_node_attri:
            x1 = np.loadtxt(directory+"%s/raw/%s_node_attributes.txt"%(ds_name,ds_name), delimiter=',',dtype=np.float64)#.reshape(-1,1)
    ...
    class_labels = np.loadtxt(directory+"%s/raw/%s_graph_labels.txt"%(ds_name,ds_name), dtype=np.int64)
...
def normalized_laplacian_W_batch(adj_matrix_batch,layer_num):
    L_batch = []
    W_batch = []
